function [tout, zout, uout, indices, sols] = loop_hybrid_simulation(z0,ctrl,p,tspan)
%Inputs:
...
function [dz, Fc] = dynamics_continuous(t,z,ctrl,p,iphase)
    ...
    if iphase == 2  % in flight phase
       ...
       C =  C_loop_jumping_leg(z,p);
       if C < 0   
        dC= dC_loop_jumping_leg(z,p);
        Fc = -500 * C - 3*dC;
        if Fc < 0
            Fc = 0;
        end
    ...
    A = A_loop_jumping_leg(z,p);                 % get full A matrix
    b = b_loop_jumping_leg(z,u,Fc,p);               % get full b vector
